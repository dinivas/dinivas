 ...
 */
function findParseableContentEndPos(
  logOutput: string,
  startPos: number
): number {
...
function parseActionLine(
  ...
  const [
    ...
    name,
    taintedStr,
    newResourceRequiredStr
  ] = match;
  const fullyQualifiedPath = [module, dataSourceStr, type, name]
 ...
 */
function readUpToChar(
  str: string,
  fromIndex: number,
  terminatorChar: string
): string | null {
  let pos = fromIndex;
  const end = str.length;
  while (pos < end) {
...
function parseAttributeLine(
  ...
  if (startPos !== -1) {
    ...
    if (
      line.substring(startPos, startPos + OLD_NEW_SEPARATOR.length) ===
      OLD_NEW_SEPARATOR
    ) {
...
function parseStdout(logOutput: string): ParseResult {
...
export const TerraformPlanParser = { parseStdout: parseStdout };
